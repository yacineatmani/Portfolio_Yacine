...
function Dashboard({ auth, projects = [], skills = [] }: DashboardPageProps) {
    ...
    const handleProjectSubmit = () => {
        // La soumission est gérée directement dans ProjectForm
        setShowProjectForm(false);
        snackbar.success('Projet ajouté avec succès !');
    };

    const handleSkillSubmit = () => {
        // La soumission est gérée directement dans SkillForm
    ...
    return (
        <div className={`min-h-screen bg-gray-50 transition-colors dark:bg-gray-900 ${darkMode ? 'dark' : ''}`}>
            ...
            <div className="flex min-h-screen">
                ...
                {/* Sidebar */}
                <aside
                    className={`fixed top-0 left-0 z-50 h-full w-80 transform border-r border-gray-200 bg-white shadow-xl transition-transform duration-300 lg:relative lg:z-0 lg:translate-x-0 dark:border-gray-700 dark:bg-gray-800 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'} `}
                >
