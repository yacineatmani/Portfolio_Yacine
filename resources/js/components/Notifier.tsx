...
function NotificationToast({ message, type, onClose }: { message: string; type: 'success' | 'error'; onClose: () => void }) {
    ...
    return (
        <div className={`fixed right-4 bottom-4 z-50 rounded-lg p-4 text-white shadow-lg ${type === 'success' ? 'bg-green-600' : 'bg-red-600'}`}>
            <div className="flex items-center gap-2">
                <span className="material-icons">{type === 'success' ? 'check_circle' : 'error'}</span>
                <span>{message}</span>
                <button onClick={onClose} className="material-icons ml-2 hover:opacity-70">
                    close
                </button>
            </div>
        </div>
